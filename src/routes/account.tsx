...
function AccountPage() {
  ...
  useEffect(() => {
    if (user === null) {
      ...
      const t = setTimeout(() => { if (!localStorage.getItem("frashioncart.session")) navigate({ to: "/login" }); }, 50);
      return () => clearTimeout(t);
    }
    if (user?.role === "brand") navigate({ to: "/seller" });
  }, [user, navigate]);
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-md px-6 py-24 text-center">
          <h1 className="font-display text-3xl mb-4">Sign in to view your account</h1>
          <Link to="/login" className="inline-block mt-4 bg-foreground px-6 py-3 text-[11px] uppercase tracking-widest text-background">Sign in</Link>
        </div>
        <SiteFooter />
      </div>
    );
      ...
      <div className="mx-auto max-w-[1100px] px-6 py-16 md:px-10">
        ...
        <div className="grid gap-10 md:grid-cols-[280px_1fr]">
          ...
          <section>
            ...
            {showForm && (
              <form onSubmit={submit} className="mb-10 border border-border p-6 space-y-4">
                ...
                {user.addresses.map(a => (
                  <div key={a.id} className="border border-border p-5">
                    <div className="flex items-start justify-between">
                      ...
                      <button onClick={() => removeAddress(a.id)} aria-label="Remove"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
